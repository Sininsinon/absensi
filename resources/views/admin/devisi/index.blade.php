                    ...
                    <div class="card-body p-2">
                        <table class="table table-hover" id="table">
                            <thead>
                                <tr>
                                    <th>No.</th>
                                    <th style="width: 70%">Tempat Devisi</th>
                                    <th>Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$item->name_divisions}}</td>
                                    <td>
                                        <form action="/devisi/{{$item->id}}" id="delete-form">
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-danger"
                                                id="{{$item->name_divisions}}" data-id="{{$item->id}}"
                                                onclick="confirmDelete(this)"><i class="fa fa-trash"></i> Delete</a>
                                        </form>
                                    </td>
                                </tr>
                                @include('admin.devisi.edit', ['id' => $item->id])
                                @endforeach
                            </tbody>
                        </table>
                    </div>
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    form.action = '/devisi/' + id; // Ubah aksi form sesuai dengan ID yang sesuai
                    form.submit();
                }
            });
    }
